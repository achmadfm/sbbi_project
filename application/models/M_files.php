...
class M_files extends CI_Model{
	...
	function tambah_sampul($kode,$sampul){
		$hsl = $this->db->query("UPDATE tbl_files SET file_sampul='$sampul' WHERE file_id='$kode'");
		return $hsl;
	}
	...
	function get_files_home(){
		$hsl=$this->db->query("SELECT file_id,file_judul,file_deskripsi,file_sampul,DATE_FORMAT(file_tanggal,'%d-%M-%Y') AS tanggal,file_download,file_data FROM tbl_files ORDER BY file_id DESC limit 7");
		return $hsl;
	}

	function get_files_perpage($offset, $limit){
		$hasil = $this->db->query("SELECT file_id,file_judul,file_deskripsi,file_sampul,DATE_FORMAT(file_tanggal,'%d-%M-%Y') AS tanggal,file_download,file_data FROM tbl_files ORDER BY file_id DESC limit $offset,$limit");
		return $hasil;
	}
