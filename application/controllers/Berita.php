...
class Berita extends CI_Controller{
    ...
    $x['page'] =$this->pagination->create_links();
		$x['category']=$this->db->query('SELECT * FROM tbl_kategori ORDER BY kategori_id DESC LIMIT 7');
	...
	function kategori(){
		 ...
		 $query = $this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%Y-%m-%d') AS tanggal FROM tbl_tulisan WHERE tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		 if($query->num_rows() > 0){
			 $x['data']=$query;
			 $x['category']=$this->db->query('SELECT * FROM tbl_kategori ORDER BY kategori_id DESC LIMIT 7');
 			 $x['populer']=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%Y-%m-%d') AS tanggal FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
	...
	function berita_detail($slugs){
		...
		if($query->num_rows() > 0){
			...
			$x['deskripsi']=cetak_meta($row['tulisan_isi'],0,500);
