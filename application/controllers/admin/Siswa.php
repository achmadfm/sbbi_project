...
class Siswa extends CI_Controller{
	...
	function simpan_siswa(){
				$config['upload_path'] = './template/students/'; //path folder
        ...
        $config['encrypt_name'] = TRUE; //nama yang terupload nantinya
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
                      ...
                      $config['source_image']='./template/students/'.$gbr['file_name'];
                      $config['create_thumb']= FALSE;
                      $config['maintain_ratio']= FALSE;
                      $config['quality']= '100%';
                      $config['width']= 300;
                      $config['height']= 300;
                      $config['new_image']= './template/students/'.$gbr['file_name'];
	...
	function hapus_siswa(){
		$kode=$this->input->post('kode');
		$gambar=$this->input->post('gambar');
		$path='./assets/images/students/'.$gambar;
		unlink($path);
